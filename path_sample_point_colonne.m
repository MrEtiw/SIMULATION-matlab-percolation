...
% La sortie (percolation) donne 1 si le chemin est complété et sort 0 s'il
% ne l'est pas.

% Depart doit etre un vecteur colonne avec x et y en entree. Arrive est le numero de la colonne d'arret. M est la matrice etudiee

function [Q,percolation] = path_sample_point_colonne(M,depart,arrive)
...
while condition
    ...
    % Vérification qu'on est rendu dans la colonne d'arrivée
    somme_depart_dans_Q = sum(Q(2,:)==arrive);
    
    if size(Q,2) == b 
        condition = false;
        percolation = 0;
    elseif somme_depart_dans_Q > 0
        condition = false;
        percolation = 1;
    end
